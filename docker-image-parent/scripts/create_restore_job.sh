...
if [ "$(kubectl get pod --no-headers | grep turbo-restore)" != '' ] ; then
    echo 'ERROR: There is a restore already running, ending script'
    exit 1
fi
if [ "$(kubectl get pod --no-headers | grep turbobkup)" != '' ] ; then
...
kubectl apply -f dbrestore.yaml

# make sure the pod starts and runs
echo '=============== wait for pod to start ==============='
count=0
while [ "$(kubectl get pod --no-headers | grep turbo-restore | awk '/Completed|Running/ {print $0}' )" = '' -a ${count} -lt ${startPodWait} ] ; do
    echo "${count}/${startPodWait}: waiting for job pod to start"
    sleep ${period}
    count=$(expr ${count} + 1)
done
if [ "$(kubectl get pod --no-headers | grep turbo-restore | awk '/Completed|Running/ {print $0}' )" = '' ] ; then
    ...
    count=0
    while [ "$(kubectl get pod --no-headers | grep Completed | grep turbo-restore | cut -f1 -d' ')" = '' -a ${count} -lt ${maxPodWait} ] ; do
    ...
    echo '=============== Logs ==============='
    if [ "$(kubectl get pod --no-headers | grep Completed | grep turbo-restore | cut -f1 -d' ')" = '' ] ; then
        echo 'ERROR: Pod failed to complete successfully'
        kubectl delete -f dbrestore.yaml
        failed=1
    fi
    kubectl logs $(kubectl get pod --no-headers | grep Completed | grep turbo-restore | cut -f1 -d' ')
    echo '============= Logs END ============='
fi
