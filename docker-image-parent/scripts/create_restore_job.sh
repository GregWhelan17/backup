...
count=0
while [ "$(kubectl get pod --no-headers | grep db-restore-job | awk '/Completed|Running/ {print $0}' )" = '' -a ${count} -lt ${startPodWait} ] ; do
...
if [ "$(kubectl get pod --no-headers | grep db-restore-job | awk '/Completed|Running/ {print $0}' )" = '' ] ; then
    ...
    count=0
    while [ "$(kubectl get pod --no-headers | grep Completed | grep db-restore-job | cut -f1 -d' ')" = '' -a ${count} -lt ${maxPodWait} ] ; do
